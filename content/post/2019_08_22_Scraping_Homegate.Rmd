---
title: "Scraping Homegate.ch real estate data"
...
replace_umlauts <- function(string){
  string %>%
    gsub("ä","a",.,ignore.case = TRUE) %>%
    gsub("ö","o",.,ignore.case = TRUE) %>%
    gsub("ü","u",.,ignore.case = TRUE) %>%
    gsub("è","e",.,ignore.case = TRUE) %>%
    gsub("é","e",.,ignore.case = TRUE) %>%
    gsub("à","a",.,ignore.case = TRUE) %>%
    gsub("ë","e",.,ignore.case = TRUE)
}


# Takes the direkt link to the wohnung and returns a dataframe with title and description of the wohnung
get_full_description <- function(url){
  all <- read_html(url) %>%
    html_nodes(".detail-description")
  ...
  data_frame(title = title, text = text)
}
