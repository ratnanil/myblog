...
If View were a generic, we could just define a custom method to said generic and *voilà*. Unfortunately, `View` is *not* a generic, you can check this by typing `View` into the console  and comparing it to `plot` (which is a generic).
...
## <environment: 0x0000025a2344eec8>
```
...
We can turn `View` into a generic with `UseMethod()`:

(Hadley states in his book that writing a method is considered "exceedingly bad manners" if you don't own the generic or the class. But since I doing all this in my own, private R-Session this is probably OK)
...
Then, we can define a default method for `View`. We can't use `utils::View` for this, since Rstudio overrides `View` on the search path (within the package `utils` environment). Since it's not overwritten in the utils namespace, we can use `get`. 

(This is taken from [this stack-overflow Question](https://stackoverflow.com/q/41532300/4139249)).
...
Using `View()` on `sf`-objects should now be much faster and more lightweight. To use this custom Version of `View()` in every R Session, I will now add the above code lines to my [.Rprofile](https://rstats.wtf/r-startup.html#rprofile). You can edit your rprofile by typing `usethis::edit_r_profile()` into the console if you have the package `usethis`, or [locating the file manually](https://stackoverflow.com/q/13735745/4139249) if you don't.
